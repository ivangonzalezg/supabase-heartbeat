...
/api/auth/reference                      # Better Auth's native Scalar reference UI
```
...
* The NestJS document, generated by `@nestjs/swagger` from decorated
  controllers (`GET /api/health`, tagged `health`).
...
retags every Better Auth operation as `Authentication`, so the single page
groups endpoints by tag (`health`, `Authentication`) instead of needing a
document selector. Better Auth's document is OpenAPI 3.1-shaped (it uses
the `type: [T, "null"]` nullable form, invalid under 3.0), so the merged
...
`/api/auth/open-api/generate-schema`, and its native reference UI at
`/api/auth/reference` (Scalar, provided by the same plugin) — useful if you
want Better Auth's endpoints in isolation, but `/api/docs` is the complete
picture.
